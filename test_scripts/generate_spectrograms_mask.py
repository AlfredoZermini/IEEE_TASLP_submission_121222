...
def generate_spectrograms_mask(spectrograms_left, spectrograms_right, target_original0, target_original1, target_original2, mask_target0, mask_target1, mask_target2, sound_path, figures_path ):
    ...
    for angle in angles_list:
        ...
        path = os.path.join(sound_path, str(angle0) + 'deg_' + str(angle1) + 'deg_' + str(angle2) + 'deg'  )
        if not os.path.exists(path):
            os.makedirs(path)
        print(target_separated_L0.shape, image)
        ...
        name = path + '/spectrogram_L_'+ str(angle0) + 'deg_' + str(angle1) + 'deg_' + str(angle2) + 'deg.wav'
        wavfile.write(name,fs,spectrogram_L_sound)

        [_, target_separated_R0_sound] = signal.istft(target_separated_R0[image,:,:], fs, 'hann', Wlength, overlap, fft_size)
        name = path + '/target_separated_R0_'+ str(angle0) + 'deg.wav'
        wavfile.write(name,fs,target_separated_R0_sound)

        [_, target_separated_R1_sound] = signal.istft(target_separated_R1[image,:,:], fs, 'hann', Wlength, overlap, fft_size)
        name = path + '/target_separated_R1_'+ str(angle1) + 'deg.wav'
        wavfile.write(name,fs,target_separated_R1_sound)

        [_, target_separated_R2_sound] = signal.istft(target_separated_R2[image,:,:], fs, 'hann', Wlength, overlap, fft_size)
        name = path + '/target_separated_R2_'+ str(angle2) + 'deg.wav'
        wavfile.write(name,fs,target_separated_R2_sound)

        [_, spectrogram_R_sound] = signal.istft(spectrograms_right[image,:,:], fs, 'hann', Wlength, overlap, fft_size)
        name = path + '/spectrogram_R_'+ str(angle0) + 'deg_' + str(angle1) + 'deg_' + str(angle2) + 'deg.wav'
        wavfile.write(name,fs,spectrogram_R_sound)
    ...
    return target_separated_L0, target_separated_R0, target_separated_L1, target_separated_R1, target_separated_L2, target_separated_R2

def generate_spectrograms_mask4(spectrograms_left, spectrograms_right, target_original0, target_original1, target_original2, target_original3, mask_target0, mask_target1, mask_target2, mask_target3, sound_path, figures_path ):
