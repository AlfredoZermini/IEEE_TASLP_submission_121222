...
def evaluate_performance_stereo(matlab_path, p0, target_separated, baseline, n_sources_string):
    ...
    for sample in range(0, n_samples):
        ...
        if baseline not in my_methods:
            ...
            for source in range(n_sources):
                ...
                if write_audio == True:
                    ...
                    target0 = np.vstack((target_L0, target_R0)).T    
                    target1 = np.vstack((target_L1, target_R1)).T
                    target2 = np.vstack((target_L2, target_R2)).T
                    ...
                    if n_sources == 4:
                        ...
                        target3 = np.vstack((target_L3, target_R3)).T
        ...
        for ch in range(0,2):
            ...
            # baselines channels need to be swapped
            if baseline not in my_methods:
            ...
            # targets
            target0_ch_path = os.path.join(stereo_separated_path, 'Est_s'+ str(0) + '_' + str(sample%n_mixtures) + '_' + str(ch0) + '.wav')
            soundfile.write(target0_ch_path, target0[ch0, 0:tmax], fs )
            
            target1_ch_path = os.path.join(stereo_separated_path, 'Est_s'+ str(1) + '_' + str(sample%n_mixtures) + '_' + str(ch0) + '.wav')
            soundfile.write(target1_ch_path, target1[ch0, 0:tmax], fs )

            target2_ch_path = os.path.join(stereo_separated_path, 'Est_s'+ str(2) + '_' + str(sample%n_mixtures) + '_' + str(ch0) + '.wav')
            soundfile.write(target2_ch_path, target2[ch0, 0:tmax], fs )


            # write audio if do not exist
            ### ch+1 because there are 3 channels
            [_, mixture] = signal.istft(p0_mix[sample,:,:,ch+1], fs, 'hann', Wlength, overlap, fft_size)
            soundfile.write(mixture_path, mixture[0:tmax], fs )

            # source 0
            [_, original0] = signal.istft(p0[sample,:,:,0,ch+1], fs, 'hann', Wlength, overlap, fft_size)
            soundfile.write(original0_path, original0[0:tmax], fs )
            
            # source 1
            [_, original1] = signal.istft(p0[sample,:,:,1,ch+1], fs, 'hann', Wlength, overlap, fft_size)
            soundfile.write(original1_path, original1[0:tmax], fs )

            # source 2
            [_, original2] = signal.istft(p0[sample,:,:,2,ch+1], fs, 'hann', Wlength, overlap, fft_size)
            soundfile.write(original2_path, original2[0:tmax], fs )


            if n_sources == 4:  # source 3

                target3_ch_path = os.path.join(stereo_separated_path, 'Est_s'+ str(3) + '_' + str(sample%n_mixtures) + '_' + str(ch0) + '.wav')
                soundfile.write(target3_ch_path, target3[ch0, 0:tmax], fs )

                [_, original3] = signal.istft(p0[sample,:,:,3,ch+1], fs, 'hann', Wlength, overlap, fft_size)
                soundfile.write(original3_path, original3[0:tmax], fs )

            ### EVALUATE METRICS
            if n_sources == 3:
                sdr, sir, sar = eng.bss_eval3_wrapper(target0_ch_path, target1_ch_path, target2_ch_path, original0_path, original1_path, original2_path, nargout=3)
                sdr_mix, sir_mix, sar_mix = eng.bss_eval3_mix_wrapper(mixture_path, original0_path, original1_path, original2_path, nargout=3)
                #print(sdr)
                
            elif n_sources == 4:
                sdr, sir, sar = eng.bss_eval4_wrapper(target0_ch_path, target1_ch_path, target2_ch_path, target3_ch_path, original0_path, original1_path, original2_path, original3_path, nargout=3)
            ...
            pesq0 = eng.pesq(original0_path,target0_ch_path);
            pesq0_mix = eng.pesq(original0_path,mixture_path);

            pesq1 = eng.pesq(original1_path,target1_ch_path);
            pesq1_mix = eng.pesq(original1_path,mixture_path);

            pesq2 = eng.pesq(original2_path,target2_ch_path);
            ...
            if n_sources == 3:
                ...
                pesq3 = eng.pesq(original3_path,target3_ch_path);
