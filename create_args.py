...
def create_args():
    ...
    args.threshold = config['audio']['threshold']
    args.n_time = config['audio']['n_time']
    ...
    args.n_doas = config['DOA']['n_doas']
    args.n_channels = config['DOA']['n_channels']
    args.n_utt = config['DOA']['n_utt']
    args.n_mix = args.n_utt*args.n_doas_mix #22500 train

    # baselines
    args.generate_mat_files = config['matlab']['generate_mat_files']
    ...
    ### datasets paths
    args.TIMIT_path = os.path.join(args.STORAGE_PATH, 'datasets', 'TIMIT')
    args.rir_path = os.path.join(args.STORAGE_PATH, 'datasets', 'B_format_RIRs_Alfredo_S3A', 'Room' + args.Train_Room)

    # train
    args.TIMIT_train_path = os.path.join(args.TIMIT_path, 'train')

    # test
    args.TIMIT_test_path = os.path.join(args.TIMIT_path, 'test')
    ...
    args.load_test_data_path = os.path.join(args.load_train_room_path.replace('TrainData','TestData'), args.n_sources_string + 'Sources' + args.post_suffix)
    ...
    args.data_val_path = os.path.join(args.load_val_data_path, 'audio')
    args.data_test_path = os.path.join(args.load_test_data_path, 'audio')
